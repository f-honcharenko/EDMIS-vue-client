...
<b-button v-b-modal.modal-prevent-closing>New User</b-button>
...
export default {
  ...
  methods: {
    ...
    handleOk(){
      ...
      axios.post(this.$nodeLink + "/api/user/register", { login:this.login,lname:this.lname,fname:this.fname,password:this.password }).then(
      (res) => {
        if (res.status == 201) {
          // this.auth = true;
